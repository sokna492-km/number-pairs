...
// 2. Build the updated script with cute & minimalist toggle design
const modernInjection = `  <!-- Kantumruy Pro Font & Minimalist Language Toggle -->
  ...
  <style id="kantumruy-custom-styles">
    ...
    * {
    ...
    /* Cute, minimalist floating language switch */
    #top-lang-toggle-bar {
      position: fixed;
      top: 14px;
      right: 16px;
      ...
      box-shadow: 0 4px 16px -2px rgba(99, 102, 241, 0.12), 0 2px 6px -1px rgba(0, 0, 0, 0.04);
      pointer-events: auto !important;
      touch-action: manipulation;
      user-select: none;
      -webkit-user-select: none;
      transition: opacity 0.25s cubic-bezier(0.34, 1.56, 0.64, 1), transform 0.25s cubic-bezier(0.34, 1.56, 0.64, 1), visibility 0.25s cubic-bezier(0.34, 1.56, 0.64, 1);
      opacity: 1;
      visibility: visible;
    ...
    #top-lang-toggle-bar.sim-screen-hidden {
      opacity: 0 !important;
  ...
  <script id="kantumruy-and-locale-controller">
    ...
    window.setToggleVisibility = function(isHome) {
      const bar = document.getElementById('top-lang-toggle-bar');
      if (!bar) return;
      if (isHome) {
    ...
    // Continuous background monitor to ensure visibility stays strictly accurate
...
console.log('Successfully updated index.html with cute and minimalist toggle!');
