...
// 2. Build the updated script with exact requested Dropdown UI
const dropdownInjection = `  <!-- Kantumruy Pro Font & Minimalist Language Toggle -->
  ...
  <script id="kantumruy-and-locale-controller">
    ...
    window.switchLanguage = function(locale) {
      ...
      // Fallback: If localeProperty is not ready yet, keep checking
      if (!switched) {
        let attempts = 0;
    ...
    window.renderDropdownUI = function(currentLocale) {
      ...
      if (btnTrigger) {
        btnTrigger.addEventListener('click', function(e) {
          ...
          e.stopPropagation();
          isOpen = !isOpen;
          window.renderDropdownUI(window.__currentAppLocale);
        });
      }
    ...
    const syncWatcher = setInterval(function() {
      if (window.phet && window.phet.joist) {
        ...
        if (window.phet.joist.localeProperty) {
          ...
          window.renderDropdownUI(window.phet.joist.localeProperty.value);
          if (!window.phet.joist.__localeLinked) {
            ...
            window.phet.joist.localeProperty.lazyLink(function(newVal) {
              ...
              window.renderDropdownUI(newVal);
              try {
                const url = new URL(window.location.href);
...
console.log('Successfully updated index.html with requested dropdown component UI!');
